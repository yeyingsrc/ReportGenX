...
def list_icp_entries():
    """获取所有 ICP 信息 (直接读取数据库)"""
    return get_db_reader().read_icp_raw_list()

@app.delete("/api/icp-entry/{vuln_id}")
def delete_icp_entry(vuln_id: str):
    """删除指定的 ICP 信息 (根据 Vuln_id)"""
    success, msg = get_db_reader().delete_icp_entry(vuln_id)
...
def add_icp_entry(req: IcpEntryRequest):
    """新增 ICP 信息"""
    success, msg = get_db_reader().add_icp_entry(req.dict())
    if success:
        return {"success": True, "message": msg}
    raise HTTPException(status_code=500, detail=msg)

@app.put("/api/icp-entry/{vuln_id}")
def update_icp_entry(vuln_id: str, req: IcpEntryRequest):
    """更新 ICP 信息"""
    success, msg = get_db_reader().update_icp_entry(vuln_id, req.dict())
    if success:
        return {"success": True, "message": msg}
    raise HTTPException(status_code=500, detail=msg)
